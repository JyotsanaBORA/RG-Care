...
# Run this ON the VPS: bash ~/deploy.sh OR bash ~/deploy_rgcare.sh
#
# What it does:
#   1. Clone or git pull latest code from GitHub (https://github.com/JyotsanaBORA/RG-Care.git)
...
REPO_URL="https://github.com/JyotsanaBORA/RG-Care.git"
...
SELF_IN_REPO="$RGCARE_DIR/deploy.sh"
SELF_IN_HOME="$HOME/deploy.sh"
...
# ── 1. Pull latest code / Clone repository ────────────────────────────────────
echo "==> [1/6] Fetching latest code from GitHub ($REPO_URL)..."
if [ ! -d "$RGCARE_DIR/.git" ]; then
  echo "    Repository not found locally at $RGCARE_DIR. Cloning..."
  git clone "$REPO_URL" "$RGCARE_DIR"
  cd "$RGCARE_DIR"
else
  cd "$RGCARE_DIR"
...
if [ -f "$SELF_IN_REPO" ] && [ -f "$SELF_IN_HOME" ] && ! diff -q "$SELF_IN_REPO" "$SELF_IN_HOME" > /dev/null 2>&1; then
  echo "==> deploy.sh updated — re-running new version..."
